...
- Task 1-3: completed with existing collected data and markdown conversion.
- Task 4: completed local recursive chunking + TF-IDF/SVD vector index.
- Task 5: completed semantic search.
- Task 6: completed BM25 lexical search.
- Task 7: completed reranking with RRF/MMR/local cross-encoder style scorer.
- Task 8: completed PageIndex-compatible local vectorless fallback.
- Task 9: completed hybrid retrieval pipeline with fallback.
- Task 10: completed generation with citation and lost-in-the-middle reordering.

### Group Evaluation Deliverables

- `group_project/evaluation/golden_dataset.json`: 15 Q&A pairs.
- `group_project/evaluation/eval_pipeline.py`: runnable offline evaluation script.
- `group_project/evaluation/results.md`: A/B scores, worst performers, recommendations.
- A/B configs: `hybrid_rerank` vs `dense_only`.

### Architecture

```text
data/standardized/*.md
  -> Task 4 chunking + local TF-IDF/SVD index
  ...
  -> golden_dataset.json + results.md
```

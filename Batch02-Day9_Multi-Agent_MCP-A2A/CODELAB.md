...
LLM không được khởi tạo trực tiếp bằng các tham số API (như OpenAI hay Anthropic) ngay trong file, mà được gọi thông qua một hàm có sẵn tên là get_llm().

Hàm này được import từ một module nội bộ của dự án:
from common.llm import get_llm
Khởi tạo: Biến llm được gán bằng kết quả của hàm này tại dòng:
llm = get_llm()
2. Message được gửi đến LLM có cấu trúc gì?
Message được gửi đến LLM có cấu trúc là một danh sách (list) chứa các đối tượng tin nhắn (message objects) của thư viện LangChain. Cụ thể ở đây, danh sách này gồm 2 phần tử:
...
Sau đó, toàn bộ danh sách này được gửi tới LLM thông qua phương thức bất đồng bộ: await llm.ainvoke(messages).
3. Tại sao cần có `SystemMessage` và `HumanMessage`?
Việc chia thành các loại tin nhắn khác nhau giúp phân định rõ ràng các luồng thông tin cung cấp cho mô hình AI, từ đó AI sẽ phản hồi chính xác hơn:

SystemMessage (Tin nhắn hệ thống): Dùng để thiết lập vai trò, bối cảnh, và các quy tắc/ràng buộc cho AI.

Trong file này: Nó yêu cầu AI đóng vai một chuyên gia pháp lý ("You are a legal expert"), đưa ra phân tích rõ ràng, súc tích và giới hạn câu trả lời dưới 300 từ. AI sẽ luôn ghi nhớ chỉ thị nền tảng này trong suốt quá trình trả lời.

HumanMessage (Tin nhắn người dùng): Chứa câu hỏi hoặc yêu cầu thực tế mà người dùng muốn AI giải quyết.

Trong file này: Nó chứa nội dung của biến QUESTION (ví dụ: "Hậu quả pháp lý nếu một công ty vi phạm thỏa thuận bảo mật là gì?").

Sự tách biệt này giúp LLM không bị nhầm lẫn giữa những quy tắc nó phải tuân thủ và câu hỏi nó cần phải trả lời.
...
Trong file code, decorator @tool được import từ thư viện langchain_core.tools và được đặt ngay trên đầu hàm search_legal_knowledge(query: str) và hàm calculate_damages(contract_value: float, breach_severity: str). Nó biến các hàm Python thông thường này thành các công cụ mà AI có thể hiểu được cấu trúc (schema).
2. `LEGAL_KNOWLEDGE` được cấu trúc như thế nào?
Nó là một danh sách các từ điển (List[Dict]), mỗi phần tử đại diện cho một bản ghi dữ liệu pháp lý gồm 3 trường: id (mã định danh), keywords (các từ khóa kích hoạt), và text (nội dung chi tiết của điều luật). Đây chính là một Vector Store/Knowledge Base giả lập phục vụ cho cơ chế RAG.
3. LLM được bind với tools ra sao? (Tìm `.bind_tools()`)
tools = [search_legal_knowledge, calculate_damages]
llm_with_tools = llm.bind_tools(tools)
Hàm .bind_tools(tools) truyền trực tiếp danh sách công cụ vào mô hình LLM để ép mô hình khi nhận câu hỏi phải tự động so khớp cấu trúc xem có cần trích xuất tham số gọi hàm hay không.

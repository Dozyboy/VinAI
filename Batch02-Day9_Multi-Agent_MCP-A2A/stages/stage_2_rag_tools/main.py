...
TOOLS = [check_statute_of_limitations, search_legal_database, calculate_damages]

# QUESTION = "What are the legal consequences if a company breaches a non-disclosure agreement?"
QUESTION = (
    "Nhân viên bị tai nạn đã điều trị 12 tháng chưa khỏi, công ty có thể đơn phương "
    "chấm dứt hợp đồng lao động không? Nếu có tranh chấp hợp đồng thì thời hiệu khởi kiện là bao lâu?"
)
# QUESTION = "Tôi có một hợp đồng thương mại trị giá 500,000 đô la. Đối tác của tôi đã vi phạm nghiêm trọng (severe breach) hợp đồng này. Hãy cho tôi biết các biện pháp khắc phục theo luật UCC và tính toán sơ bộ khoản tiền bồi thường thiệt hại mà tôi có thể nhận được."
# Test tool check_statute_of_limitations:
# QUESTION = "Tôi đang chuẩn bị khởi kiện một vụ án liên quan đến tranh chấp tài sản (property) và một vụ vi phạm hợp đồng (contract). Hãy cho tôi biết thời hiệu khởi kiện của từng loại vụ án là bao lâu?"
# Test cả check_statute_of_limitations và search_legal_database:
# QUESTION = "Nhân viên của tôi bị tai nạn đã điều trị 12 tháng chưa khỏi, tôi có quyền đơn phương chấm dứt hợp đồng lao động không? Nếu sau này nhân viên đó kiện tôi vì vi phạm hợp đồng thì thời hiệu khởi kiện là bao lâu?"
...
async def main():
    ...
    print()
    print("[How it works]")
    print("  1. LLM receives tools (check_statute_of_limitations, search_legal_database, calculate_damages)")

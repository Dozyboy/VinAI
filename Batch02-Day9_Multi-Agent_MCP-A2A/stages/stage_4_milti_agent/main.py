...
def search_privacy_law(query: str) -> str:
    ...
    Args:
        query: Natural language query about privacy, data protection, or GDPR.
    """
    knowledge = [
        (
            ["data", "privacy", "gdpr", "breach", "consent", "user"],
            "GDPR requires a lawful basis for processing personal data, transparency, data "
            "minimisation, and appropriate security controls. Serious violations can trigger "
            "administrative fines up to EUR 20M or 4% of global annual turnover.",
        ),
        (
            ["breach", "notification", "leak", "customer", "personal"],
            "Data breach response typically requires prompt containment, forensic review, "
            "regulator notification, and affected-user notice when risk thresholds are met. "
            "Delays or incomplete notices can increase enforcement exposure.",
        ),
        (
            ["ccpa", "cpra", "california", "consumer"],
            "CCPA/CPRA creates consumer rights to know, delete, correct, and opt out of "
            "certain sharing/sale of personal information. Intentional violations can reach "
            "$7,500 per violation.",
        ),
    ]
    query_lower = query.lower()
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves personal data, privacy, GDPR, CCPA, data breaches'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a senior privacy attorney with expertise in GDPR, CCPA/CPRA, consent, "
        "data breach response, cross-border transfers, data subject rights, and privacy "
        "regulator enforcement. Use the search_privacy_law tool to ground your analysis. "
...
QUESTION = (
    "If a company breaks a contract, avoids taxes, and leaks customer personal data, "

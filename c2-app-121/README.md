...
### Bước 7: Khởi chạy Frontend React/Vite (Cổng 5173)
Mở thêm một terminal mới, di chuyển vào thư mục frontend và cài đặt các gói phụ thuộc:
```bash
cd c2-app-121-frontend
npm install
```

Sau đó khởi chạy giao diện frontend:
```bash
npm run dev
```
...
> **Lưu ý:** Frontend đã cấu hình proxy `/api/v1` sang backend `http://localhost:8000`, vì vậy hãy đảm bảo MCP Clinical Server và FastAPI Backend đang chạy trước khi thao tác trên giao diện.

---

## 🚢 Hướng dẫn Deploy Production

Xem hướng dẫn chi tiết để deploy backend lên Railway, frontend lên Vercel và lấy production URL tại: [docs/deployment.md](docs/deployment.md).

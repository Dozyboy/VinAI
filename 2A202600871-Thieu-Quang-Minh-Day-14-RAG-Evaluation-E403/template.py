...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for rank, chunk in enumerate(contexts, start=1):
            ...
            coverage = len(chunk_tokens & expected_tokens) / expected_len
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "Return JSON with criterion names mapped to scores from 0 to 1, "
        ...
        try:
            ...
            if isinstance(parsed, dict):
                source_scores = parsed.get("scores", parsed)
                if isinstance(source_scores, dict):
                    scores = {
                        criterion: max(0.0, min(1.0, float(source_scores[criterion])))
                        for criterion in rubric
                        if criterion in source_scores
                        and isinstance(source_scores[criterion], (int, float))
                    }
                    for criterion in rubric:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def mean_score(value: Any) -> float | None:
        ...
        for result in scores_batch:
            ...
            score_values = scores.values() if isinstance(scores, dict) else [scores]
            all_scores.extend(
                float(score)
            ...
            first_score = (
                result.get("first_score")
                if "first_score" in result
                else result.get("first_scores")
            )
            second_score = (
                result.get("second_score")
                if "second_score" in result
                else result.get("second_scores")
            )
            first_avg = mean_score(first_score)
            second_avg = mean_score(second_score)
            if first_avg is not None and second_avg is not None:
                pair_count += 1
                first_higher_count += first_avg > second_avg

            position = result.get("position", result.get("response_position"))
            score_avg = mean_score(scores)
            if score_avg is not None and isinstance(position, str):
                if position.lower() in {"first", "1", "a", "response_1"}:
                    first_position_scores.append(score_avg)
                else:
                    other_position_scores.append(score_avg)

        if pair_count:
            positional_bias = first_higher_count / pair_count >= 0.8
        elif first_position_scores and other_position_scores:
            positional_bias = (
                sum(first_position_scores) / len(first_position_scores)
                > sum(other_position_scores) / len(other_position_scores)
            )
        else:
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review failure and add targeted fix"
            )
            rows.append(
                "| F{index:03d} | {failure_type} | {root_cause} | {suggestion} | Open |".format(
                    index=index,
                    failure_type=self._clean_cell(failure.failure_type or "unknown"),
                    root_cause=self._clean_cell(self.find_root_cause(failure)),
                    suggestion=self._clean_cell(suggestion),
                )
            )
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": "Add faithfulness guardrails and require every factual claim to be grounded in retrieved context",
            "irrelevant": "Tighten the prompt so the answer directly addresses the user question before adding extra detail",
            "incomplete": "Improve retrieval coverage and add few-shot examples that demonstrate complete answers",
            "off_topic": "Add intent classification and reject or clarify questions outside the supported domain",
            "refusal": "Relax refusal rules for safe in-domain questions and add examples of allowed answers",
            "unknown": "Review uncategorized failures and add labels before optimizing the pipeline",
        }

        ordered_types = sorted(categories, key=lambda item: (-categories[item], item))
        suggestions = [
            suggestions_by_type.get(
                failure_type,
                f"Investigate {failure_type} failures and add a targeted regression test",
            )
        ...
        if any("retrieval" in cause for cause in root_causes):
            suggestions.append("Tune chunk size, top-k, and reranking to surface stronger supporting evidence")
        if any("prompt" in cause for cause in root_causes):
            suggestions.append("Add a response checklist to the generation prompt for relevance and scope control")
        if any("context window" in cause or "generation" in cause for cause in root_causes):
            suggestions.append("Summarize or compress retrieved context so key evidence fits in the prompt")
